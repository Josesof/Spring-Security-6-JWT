...
public class JwtTokenProvider {
  ...
  public String generateToken(Authentication authentication){
    ...
    Date expirationToken = new Date(timeNow.getTime() + ConstSecurityConfig.JWT_EXPIRATION_TOKEN);
    ...
    String token = Jwts.builder() //Construimos un token JWT llamado token
      ...
      .signWith(SignatureAlgorithm.HS512, ConstSecurityConfig.JWT_FIRMA) /*Utilizamos este método para firmar
  ...
  //Metodo para extraer username apartir de un token
  public String getUsernameofJwt(String token){
    Claims claims = Jwts.parser() // El método parser se utiliza con el fin de analizar el token
      .setSigningKey(ConstSecurityConfig.JWT_FIRMA) // Establece la clave de firma, que se utiliza para verificar la firma del token
  ...
  public Boolean validateToken(String token){
    try{
      ...
      //Si son idénticas validara el token o caso contrario saltara la excepción de abajo
           Jwts.parser().setSigningKey(ConstSecurityConfig.JWT_FIRMA).parseClaimsJws(token);
              return  true;
    }catch (Exception e){
